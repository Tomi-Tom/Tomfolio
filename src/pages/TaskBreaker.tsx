...
export default function TaskBreaker(): ReactElement {
  ...
  const addStep = () => {
    ...
    setActiveTask(
      updatedTasks.find((task) => task.id === activeTask.id) || null
    )
  }
  ...
  const toggleStepCompletion = (taskId: string, stepId: string) => {
    const updatedTasks = tasks.map((task) => {
      ...
      return {
        ...
        steps: updatedSteps,
        completedAt:
          allStepsCompleted && !task.completedAt
        ...
        >
          <div className="mb-12 text-center">
            ...
            <p className="text-secondary mx-auto mb-8 max-w-2xl text-lg">
              ...
              Designed specifically for individuals with ADHD to reduce task
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-12">
            ...
            <div className="md:col-span-5 lg:col-span-4">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-gold text-xl font-bold">My Tasks</h2>
                <motion.button
                ...
                {(['all', 'active', 'completed'] as const).map((status) => (
                  <button
                    ...
                    className={`flex-1 rounded-md px-4 py-2 text-sm transition-colors ${
                      filterStatus === status ? 'text-gold' : 'text-secondary'
                    }`}
                    style={
                      filterStatus === status
                        ? {
              ...
              {/* Tasks List */}
              <div className="void-panel max-h-[600px] min-h-[400px] overflow-y-auto rounded-xl p-4">
                  ...
                  <ul className="space-y-3">
                    <AnimatePresence>
                      {filteredTasks.map((task, index) => (
                        <motion.li
                          key={task.id}
                          className={`void-panel cursor-pointer rounded-lg p-3 ${
                        ...
                        >
                          <div className="flex justify-between">
                            <div className="flex items-start">
                              <div
                                className="mt-1 mr-2 h-4 w-4 flex-shrink-0 rounded-full border"
                                style={
                                  task.completedAt
                                    ? {
                              ...
                              <div>
                                <h4
                                  className={`${task.completedAt ? 'text-dim line-through' : 'text-gold'}`}
                                >
                                  {task.title}
                                </h4>
                                <p className="text-secondary text-xs">
                                  ...
                                  {
                                    task.steps.filter((s) => s.isCompleted)
                                      .length
                                  }{' '}
                                  completed
                                </p>
                            ...
                            <div className="flex items-center space-x-1">
                              <button
                                ...
                                className="text-secondary p-1 transition-colors"
                          ...
                          {task.isExpanded && task.steps.length > 0 && (
                            <div className="mt-3 ml-6 space-y-1">
                              {task.steps.slice(0, 3).map((step) => (
                                <div
                                  key={step.id}
                                  className={`text-sm ${step.isCompleted ? 'text-dim line-through' : 'text-secondary'}`}
                                >
                                  - {step.text}
                                </div>
                              ))}
              ...
              {/* Task Stats */}
              <div className="void-panel mt-4 rounded-xl p-4">
                <h3 className="text-gold mb-3 text-lg font-semibold">
                  Progress
                </h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="void-panel rounded-lg p-3">
            ...
            <div className="md:col-span-7 lg:col-span-8">
              {!activeTask ? (
                <div className="void-panel flex h-full flex-col items-center justify-center rounded-xl p-8 text-center">
                  <h3 className="text-gold mb-3 text-xl font-bold">
                    Task Breakdown
                  </h3>
                  <p className="text-secondary mb-6 max-w-md">
                    Select a task from the list or create a new one to break it
                ...
                <div className="void-panel rounded-xl p-6">
                  <div className="mb-6 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-secondary text-sm">
                        ...
                        {activeTask.completedAt &&
                          ` - Completed ${new Date(activeTask.completedAt).toLocaleDateString()}`}
                      </p>
                    </div>
                  ...
                  <div className="mb-6">
                    <div className="mb-1 flex justify-between text-sm">
                      ...
                      <span className="text-gold-dim">
                        {activeTask.steps.filter((s) => s.isCompleted).length}/
                        {activeTask.steps.length} steps completed
                      </span>
                    </div>
                    <div
                  ...
                  {/* Steps List */}
                  <div className="mb-6">
                    <h3 className="text-gold-dim mb-3 text-lg font-semibold">
                      Steps
                    </h3>

                    {activeTask.steps.length === 0 ? (
                      <div className="void-panel rounded-lg p-6 text-center">
                        <p className="text-secondary mb-4">
                          No steps added yet. Break down this task into smaller,
                          manageable steps.
                        </p>
                        <motion.button
                          onClick={() => setShowAddStepForm(true)}
                      ...
                      >
                        {activeTask.steps.map((step, index) => (
                          <motion.li
                            key={step.id}
                            className={`void-panel flex items-start rounded-lg p-4 ${
                          ...
                          >
                            <button
                              onClick={() =>
                                toggleStepCompletion(activeTask.id, step.id)
                              }
                              className="mt-1 mr-3 h-5 w-5 flex-shrink-0 rounded-full border"
                              style={
                                step.isCompleted
                                  ? {
                                      background: 'var(--color-gold)',
                            ...
                            <div className="ml-2 flex items-center space-x-1">
                              <button
                                onClick={() =>
                                  moveStep(activeTask.id, step.id, 'up')
                                }
                                disabled={index === 0}
                                className="p-1 transition-colors"
                                style={{
                              ...
                              <button
                                onClick={() =>
                                  moveStep(activeTask.id, step.id, 'down')
                                }
                              ...
                              <button
                                onClick={() =>
                                  deleteStep(activeTask.id, step.id)
                                }
                                className="p-1 transition-colors"
                                style={{ color: 'var(--color-text-dim)' }}
                              >
                  ...
                  <div className="void-panel rounded-lg p-4">
                    <h3 className="text-gold-dim mb-2 font-semibold">
                      Tips for Breaking Down Tasks
                    </h3>
                    <ul className="text-secondary list-inside list-disc space-y-1 text-sm">
                      <li>Make each step specific and actionable</li>
                      <li>
                        Start with the smallest possible step to build momentum
                      </li>
                      <li>
                        Use clear, concrete language ("Write intro paragraph" vs
                        "Start writing")
                      </li>
                      <li>
                        Keep steps small enough to complete in 5-15 minutes
                      </li>
                    </ul>
                  </div>
                </div>
          ...
          {showAddStepForm && activeTask && (
            ...
            >
              <motion.div
                ...
                onClick={(e) => e.stopPropagation()}
              >
                <h2 className="text-gold mb-3 text-2xl font-bold">
                  Add Steps to Task
                </h2>
                <p className="text-secondary mb-4">
                  ...
                  {bulkMode ? (
                    <div>
                      ...
                      <textarea
                        ...
                        className="input-void h-40 w-full resize-none"
                        autoFocus
                      />
                    </div>
                  ) : (
