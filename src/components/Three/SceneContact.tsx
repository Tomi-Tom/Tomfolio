...
export default function SceneContact() {
  ...
  useEffect(() => {
    ...
    const allSats = [...satellites.map(s => ({ ...s, orbitSpeed: 0.08 + Math.random() * 0.04 })),
                    ...extraSatellites.map(s => ({ ...s, orbitSpeed: 0.12 + Math.random() * 0.06, glowTex: glowMuted }))]
    ...
    allSats.forEach((sat, si) => {
      ...
      const mat = new THREE.MeshBasicMaterial({ color: sat.color, transparent: true, opacity: 0.85 })
      toDispose.push(geo, mat)
      const mesh = new THREE.Mesh(geo, mat)
      mesh.position.set(x, y, sat.z)
      scene.add(mesh)
      satMeshes.push({ mesh, mat, baseAngle: sat.angle, r: sat.r, z: sat.z, orbitSpeed: sat.orbitSpeed, phase: si * 1.2 })

      // Sat halo
      const sHaloGeo = new THREE.SphereGeometry(sat.size * 2.2, 8, 8)
      const sHaloMat = new THREE.MeshBasicMaterial({ color: sat.color, transparent: true, opacity: 0.05, side: THREE.BackSide })
    ...
    const pulses: Pulse[] = satellites.map((_, si) => {
      ...
      return { t: si / satellites.length, speed: 0.012 + Math.random() * 0.008, satIndex: si, posArr, geo, mat }
    })
    ...
    const tick = () => {
      ...
      satMeshes.forEach((sat, si) => {
        const angle = sat.baseAngle + globalAngle + Math.sin(time * 0.3 + sat.phase) * 0.05
